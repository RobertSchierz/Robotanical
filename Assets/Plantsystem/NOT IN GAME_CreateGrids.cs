...
public class CreateGrids : MonoBehaviour {
	...
	GameObject CreateChild(){
		...
		go.transform.localPosition = terrain.transform.localPosition;
	...
	void UpdatePosition(){
		...
		Physics.Raycast (transform.position, forwardPlayer, out hitInfo, Mathf.Infinity, LayerMask.GetMask ("Terrain"));
		...
		position.y -= hitInfo.point.z % cellSize + gridHeight * cellSize / 2;
	...
	bool IsCellValid(int x, int z){
		...
		Physics.Raycast (transform.TransformPoint(origin), Vector3.down, out hitInfo, Mathf.Infinity, LayerMask.GetMask ("Objects"));

		Debug.DrawRay (transform.TransformPoint(origin), Vector3.down , Color.blue);
